...
get_current_ipv4() {
  local ip
  ip=$(curl -fsS --max-time 10 https://api-ipv4.ip.sb/ip 2>/dev/null)

  if [[ ! "$ip" =~ ^([0-9]{1,3}\.){3}[0-9]{1,3}$ ]]; then
    echo "Error: Failed to get a valid IPv4 address. Got: '$ip'" >&2
    return 1
  fi
...
get_current_ipv6() {
  local ip
  ip=$(curl -fsS --max-time 10 https://api-ipv6.ip.sb/ip 2>/dev/null)

  if [[ ! "$ip" =~ : ]]; then
    echo "Error: Failed to get a valid IPv6 address. Got: '$ip'" >&2
    return 1
  fi
...
if [ "$response" -eq 200 ]; then
    ...
    if [ "$recordCount" -gt 0 ]; then
      ...
      if [ "$enableIPv4" -eq 1 ]; then
        ...
        # 获取当前IPv4地址
        currentIPv4=$(get_current_ipv4) || exit 1
...
EOF
    ...
    if [ "$recordCount" -gt 0 ]; then
      ...
      if [ "$enableIPv6" -eq 1 ]; then
        ...
        # 获取当前IPv6地址
        currentIPv6=$(get_current_ipv6) || exit 1
